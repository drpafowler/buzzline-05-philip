        ...
        with sqlite3.connect(STR_PATH) as conn:
            ...
            cursor.execute(
                ...
                INSERT INTO streamed_messages (
                    machine_id, timestamp, temperature, rpm, widgets_produced, conveyor_speed, product_quality, error_code
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    message["machine_id"],
                    ...
                    message["temperature"],
                    message["rpm"],
                    message["widgets_produced"],
                    message["conveyor_speed"],
                    message["product_quality"],
                    message["error_code"],
                ),
            )
            conn.commit()
